...
$token_from_get = isset($_GET['token']) ? trim($_GET['token']) : null;
...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'student') {
    // Если есть токен, сохраняем его для автоматической отметки после входа
    if ($token_from_get) {
        $_SESSION['pending_token'] = $token_from_get;
    }
...
// Если есть токен в GET и студент авторизован - сразу отмечаем
if ($token_from_get && $student) {
    try {
        $qr_info = getQRCodeInfo($token_from_get);
        ...
        if (!$qr_info) {
            ...
            ");
            $stmt->execute([$student['id'], $qr_info['schedule_id'], $qr_info['id']]);
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    VALUES (?, ?, ?, ?, ?, 1)
                ");
                $stmt->execute([$student['id'], $qr_info['schedule_id'], $qr_info['id'], $scan_time, $status]);
                ...
                if ($student_data['parent_email'] && $site_config['notifications_enabled']) {
                    sendNotification(
                        $student['id'],
                        $attendance_id,
...
// Обычная обработка POST формы (ручной ввод кода)
...
if ($student) {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            END as qr_exists
        FROM schedule s
        LEFT JOIN active_sessions a ON a.schedule_id = s.id 
            AND a.is_active = 1 
...
<head>
    ...
    <style>
        * {
        ...
        .btn-outline-danger { padding: 10px 20px; font-size: 16px; }
        ...
        @media (max-width: 768px) {
            ...
            h1 { font-size: 1.8rem; }
            .btn-scan { padding: 12px 16px; font-size: 16px; }
...
<body>
    ...
    <div class="container mt-3 mt-md-4 mb-4">
        <div class="text-center mb-3 mb-md-4">
            <i class="fas fa-check-circle fa-2x fa-md-3x" style="color: #667eea;"></i>
        ...
        <div class="card mb-3 mb-md-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <a href="logout.php" class="btn btn-outline-danger mt-2 mt-sm-0">
        ...
        <div class="card mb-3 mb-md-4">
            ...
            <div class="card-body p-2 p-md-3">
                            ...
                            <div class="list-group-item">
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    ...
                                    <?php if ($lesson['qr_exists']): ?>
                                        <span class="badge bg-success mt-1 mt-sm-0">QR готов</span>
                                    <?php else: ?>
                                        <span class="badge bg-secondary mt-1 mt-sm-0">QR не активен</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <div class="alert alert-info text-center mb-0">Сегодня занятий нет 🎉</div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body text-center">
                ...
                <div class="row g-2 mt-3">
                    <div class="col-6">
                        <button id="startScanBtn" class="btn btn-scan w-100">
                            <i class="fas fa-play"></i> Запустить
                        </button>
                    </div>
                    <div class="col-6">
                        <button id="stopScanBtn" class="btn btn-danger w-100" style="display:none;">
        ...
        <div class="alert alert-light mt-3 mt-md-4">
